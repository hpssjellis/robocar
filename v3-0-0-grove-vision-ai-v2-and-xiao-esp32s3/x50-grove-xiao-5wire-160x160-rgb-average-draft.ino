...
const int myTurnOk       = 200;   // 50 allowed error
const int myTurnDelay    = 10;   //15;   // was 9; was OK..... was 5; to short    // time to turn ms
...
void loop() {
    if (!AI.invoke()) {
        ...
        Serial.println();
        if (AI.boxes()[0].score > 81 && AI.boxes().size() > 3) {    // 81 gets everything, but too much informaiton
           // myMappedAngle = map(AI.boxes()[0].x, myRealMinX, myRealMaxX, myMaxAngle, myMinAngle);

            float myX, myTotal = 0.0;
            for (int i = 0; i < AI.boxes().size(); i++){
               myX += AI.boxes()[0].x;
            }
            myTotal = myX / AI.boxes().size(); 
            int myTotalInt = (int)myTotal; 
           
           // myMappedAngle = map(AI.boxes()[0].x, myRealMinX, myRealMaxX, myMinAngle, myMaxAngle);
            myMappedAngle = map(myTotalInt, myRealMinX, myRealMaxX, myMinAngle, myMaxAngle);
           // Serial.print(" score:" + String(AI.boxes()[0].score) + ", x:" + String(AI.boxes()[0].x) + ", map:" + String(myMappedAngle) + ", old:" + String(myOldMappedAngle));
            Serial.print(" score[0]:" + String(AI.boxes()[0].score) + ", x[0]:" + String(AI.boxes()[0].x) + " Boxes:" + String(AI.boxes().size()) + ", Total:" + String(myTotalInt) );
            ...
            if (myNoneCount >= myNoneMax) {   
                ...
                Serial.print(" #"); // stopped since too many none counts
            } 
        }
...
void turnMotorToPosition(int desiredPosition) {
    ...
    if (desiredPosition < 450 && currentPosition < 1000  ){
    ...
    if (abs(currentPosition - desiredPosition) > myTurnOk) {
        ...
        analogWrite(myTurnPwmPin, 0); 
        Serial.print(", |");     // no turn since current and desired position are too close.
    }
    currentPosition = analogRead(myTurnPotPin);  
