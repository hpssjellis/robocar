...
int myMaxSpeed = 75;           // set low while debugging then set higher
...
void myTask(void *parameter) {
  while (true) {
    if (myChosenSpeed > 255){myChosenSpeed = 255;}    // kind of redundant
    ...
    myMultipleObjects = 0;    // just in case we lose the connection with the Grove Vision AI V2 and this loop keeps the car going faster

    
    if (myChosenSpeed > myMaxSpeed) {myChosenSpeed = myMaxSpeed;}   // Just to be safe
...
void loop(){
    if (!AI.invoke() ){
     ...
     Serial.println();  
#endif       
     if (AI.boxes()[0].score > 80 ){    // this is the accuracy of the results
        myMultipleObjects = 0;
#ifdef DEBUG       
      ...
      // Following turn angle could probably be mapped. Students might want to try that  y = map(x, 1, 50, 50, 1);
      // int myAngle = map(AI.boxes()[0].x, 0, 320, 120, 60)   // variable, min X, max X, Max Angle, Min Angle
      // Then you would not need any if statments
