...
void myTask(void *parameter) {
  while (true) {
    ...
    if (myServoDegrees < myMinServo){myServoDegrees = myMinServo;}   // min servo angle
    if (myServoDegrees > myMaxServo){myServoDegrees = myMaxServo;} // max servo angle
    ...
    } else {     // bad connection to Grove Vision AI V2
          myAnalysisCount += 1;
          if (myAnalysisCount > 5) {
               analogWrite(myDrivePwmPin, 0);  // stop the car if no Grov  information !
               Serial.println(", Stopped, Analysis Count: " + String(myAnalysisCount)  );
               delay(500);
            }
      }
     ...
     if (AI.boxes()[0].score > 80 ){
       ...
       // analogWrite(myDrivePwmPin, myLowestSpeed);   // go medium  
        ...
        if (myMultipleObjects > 1){
           ...
           Serial.print(", Average X: " + String(myIntAverageX) );
      }  else {
      ...
      if( myIntAverageX < 90){  // just base the turn on the first object for now
            ...
            Serial.print(" Left ");
            myServoDegrees = myMinServo; // turn left
      }
        ...
        if (myStopCount > 2){    // helps detect false positives
           ...
           Serial.println(", Stopped, None Count: " + String(myStopCount)  );
        }
