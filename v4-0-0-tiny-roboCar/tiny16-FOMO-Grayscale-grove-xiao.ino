...
void loop(){
    if (!AI.invoke()){
       ...
       // Serial.println("invoke success");
        ...
        // what about stopping
        if (AI.boxes().size() == 0  || AI.boxes()[0].score < 81 ){  // yolo-v5 30 fomo 81

            if (myCountStop > 4){   // stop both motors     // yolo-v5 20, fomo 4
          ...
          // analogWrite(A0, 110); // perma set    // if objects, keep the left motor running
           ...
           // [0] is hopefully the best reading, possibly later take an average
                ...
                // Note: I messsed up and my FOMO images are upside down making the turning opposite

           if ( AI.boxes()[0].x   < 85 ){                  // yolo-v5  <145,   FOMO < 85    (FOMO numbers go up in steps of about 24 ??)
                 analogWrite(A2, 130);             // right-motor slower should turn right
                 Serial.print("RIGHT?, ");
                 digitalWrite(LED_BUILTIN, HIGH);    
           } else if ( AI.boxes()[0].x   < 120 ) {       // yolo-v5  <152,   FOMO  < 120
                analogWrite(A2, 110);            // right-motor same speed as left go straight
                Serial.print("Straight?, ");
                digitalWrite(LED_BUILTIN, LOW);    
           } else {                                     // yolo-v5   > 152  MAX probably 320,    FOMO  > 115   MAX about 220
                analogWrite(A2, 90);    
                Serial.print("LEFT?, ");
                digitalWrite(LED_BUILTIN, HIGH);    
           }
